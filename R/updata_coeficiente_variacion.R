...
coeficiente.variacion.nuevo <- function(data, variable = NULL, pesos = NULL, tipo = c("muestral", "cuasi")) {
    ...
    summarise(across(all_of(variable),
                     ~ {
                       if (is.null(pesos)) {
                         ...
                         media_ponderada <- weighted.mean(datos_completos$valores, w = datos_completos$pesos, na.rm = TRUE)
                         sd_ponderada <- sqrt(sum(datos_completos$pesos * (datos_completos$valores - media_ponderada)^2) / sum(datos_completos$pesos))
