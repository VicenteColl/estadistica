...
moda.nueva(df$marca)
moda.nueva(df,"marca")
moda.nueva(df,"mpg","wt")
moda.nueva(df$mpg,df$wt)
moda(df,"mpg","wt")
estadistica::moda(mtcars)
moda.nueva(salarios2018,"SALARIO.BRUTO.ANUAL","FACTOR.ELEVACION")
estadistica::moda(mtcars,1,pesos=6)
moda.nueva(mtcars,c("mpg","disp"),pesos="wt")
moda.nueva(df)
moda.nueva(mtcars)
...
moda.nueva(mtcars,"mpg")

tabla.bidimensional(kk,"mpg","wt") -> aa
aa$mpg <- row.names(aa)
aa <- aa[-26,c(30,31)]
aa$mpg <- as.numeric(aa$mpg)
saveRDS(aa,"aa.rds")
aa <- readRDS("aa.rds")
