...
contraste.bondad.chi <- function(x,
                                 variable = NULL,
                                 introducir = F,
  ...
  # Una posible mejora de la function es crear una funcion auxiliar que permita reagrupar las categor?as de forma
  # autom?tica para cumplir que la hip?tesis de las frecuencias esperadas sea >=5.
  ...
  if( sum(matriz$Freq_esp < 5) > 0){
    ...
    print(matriz) 
    warning("Aqu? tienes la tabla recalcudad de frecuencias esperadas porque alguna de las frecuecias teóricas era menor a 5. Para llevar a cabo el test es necesario reagrupar las categorías.")
    matriz <- agrupar(matriz$Freq_obs,matriz$Freq_esp)
    print(matriz)
  ...
  if(g.l == 1){
    ...
    estadistico.prueba <- sum((abs((matriz$Freq_obs-matriz$Freq_esp))-0.5)^2/matriz$Freq_obs)
